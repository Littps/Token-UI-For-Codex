...
export function validatePayload(payload, { allowLegacy = true } = {}) {
  ...
  for (const key of ["turnsTruncated", "requestDetailsTruncated"]) {
  ...
  if (Array.isArray(payload.turns)) {
    if (payload.turns.length > 200) errors.push("turns exceeds the 200-row limit");
    payload.turns.forEach((turn, index) => errors.push(...validateTurn(turn, index)));
  }
  if (Array.isArray(payload.requestDetails)) {
    if (payload.requestDetails.length > 100) errors.push("requestDetails exceeds the 100-row limit");
