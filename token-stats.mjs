#!/usr/bin/env node
// Tokens UI For Codex / token-stats.mjs
//
...
// 轮次摘要：内存保留全部，「推送多少」由页面的窗口请求决定（见 TURN_WINDOW_* 与 readWindowWant）。
const MAX_TURN_SUMMARIES = 500;
// 推送窗口：弹层关闭时只推最近 5 轮（极简），打开时 25 轮 + 10 条请求明细，「加载更多」每次 +50 / +20。
...
// 「有任务在跑」时把心跳加密到 1 秒：让「更新于 HH:MM:SS」与实际生成节奏同步，
// 避免长生成期间界面看起来像卡住（数值本身仍只在 record 落盘时变化）。
const ACTIVE_HEARTBEAT_INTERVAL_MS = 1000;
...
// 阈值可用环境变量覆盖（单位：分钟，供测试）：TUI_FILE_STALL_WARN_MINUTES / TUI_FILE_STALL_MINUTES。
...
const FILE_STALL_WARN_MS = minutesEnvOrDefault("TUI_FILE_STALL_WARN_MINUTES", 5);
const FILE_STALL_AFTER_MS = minutesEnvOrDefault("TUI_FILE_STALL_MINUTES", 15);
...
function processParserEvent(state, event) {
  ...
  // 轮次清单：宿主为「每一轮」单独写出 turn_context 事件，并带上该轮的 turn_id。
  // 实测（本机会话）它覆盖到日志开头、且是记录里 turn_id 的超集（还多出 5 个「无用量记录的空轮」），
  // 因此它是轮次归属的权威来源；旧日志没有该事件时才退回 user_message 分界。
  if (event.type === "turn_context" && event.payload) {
    if (Number.isFinite(event.payload.model_context_window)) {
      state.modelContextWindow = event.payload.model_context_window;
    }
    const contextTurnId = typeof event.payload.turn_id === "string" ? event.payload.turn_id : "";
    if (contextTurnId) {
      state.turnContexts.push({
        ts: timestamp,
  ...
  if (event.type === "response_item" && event.payload && event.payload.role === "user") {
    if (timestamp && !state.userMessageKeys.has(timestamp)) {
      ...
      state.userMessages.push({ ts: timestamp, ordinal: Number.isFinite(event.ordinal) ? event.ordinal : null });
    }
    return;
  }
  ...
  const payload = event.payload;
  // 运行状态：task_started 置真，task_complete / turn_aborted 置假（用于心跳分档）。
  if (payload.type === "task_started") state.taskRunning = true;
  else if (payload.type === "task_complete" || payload.type === "turn_aborted") state.taskRunning = false;
...
class IncrementalRolloutParser {
  ...
  reset() {
    ...
    // 会话日志里的任务运行状态（task_started / task_complete）
    this.taskRunning = false;
    // 轮次清单（turn_context 的 turn_id，按日志顺序）
    this.turnContexts = [];
    this.modelChanges = [];
  ...
  toParsed() {
    ...
    if (!this.counts.length && !this.usageRecords.length && !this.userMessages.length && !this.turnContexts.length) {
      this.snapshotCache = null;
      this.dirty = false;
      return null;
    }
    this.snapshotCache = {
      ...
      turnContexts: this.turnContexts,
      taskRunning: this.taskRunning === true,
...
// 请求明细：内存保留上限（=「加载更多」能翻到的深度）与推送窗口。
const MAX_REQUEST_DETAILS = 500;
...
function orderedByTimestamp(items) {
  ...
  for (let i = 1; i < ordered.length; i += 1) {
    if (compareTsOrdinal(ordered[i - 1], ordered[i]) > 0) {
      ordered.sort(compareTsOrdinal);
      break;
    }
  }
  return ordered;
...
// (ts, ordinal) 稳定比较：ordinal 是宿主写出的全局事件序号，跨分片合并后仍是全序。
// 任一侧缺 ordinal 时退化为「只比 ts」（返回 0 表示同序，交给调用方的既有规则处理）。
function compareTsOrdinal(a, b) {
  const leftTs = a && a.ts ? a.ts : "";
  const rightTs = b && b.ts ? b.ts : "";
  if (leftTs !== rightTs) return leftTs < rightTs ? -1 : 1;
  const leftOrdinal = a && Number.isFinite(a.ordinal) ? a.ordinal : null;
  const rightOrdinal = b && Number.isFinite(b.ordinal) ? b.ordinal : null;
...
// 时间轴排序：同一时间戳时 legacy 在前、新格式在后；同来源同时间戳再用 ordinal 定序。
function compareByTimestamp(a, b) {
  if (a.ts === b.ts) {
    if (a.source === b.source) {
      const leftOrdinal = Number.isFinite(a.ordinal) ? a.ordinal : null;
      const rightOrdinal = Number.isFinite(b.ordinal) ? b.ordinal : null;
      if (leftOrdinal != null && rightOrdinal != null && leftOrdinal !== rightOrdinal) {
        return leftOrdinal - rightOrdinal;
      }
      return 0;
...
function collectUnifiedRecords(parsed) {
  ...
  for (const count of parsed.counts || []) {
    ...
    legacyRecords.push({
      ...
      genMs: count.genMs,
      ts: count.ts || "",
      ordinal: count.ordinal != null ? count.ordinal : null,
      turnId: "",
      responseId: "",
      input: count.input,
      output: count.output,
  ...
  for (const record of parsed.usageRecords || []) {
    ...
    newRecords.push({
      ...
      genMs: record.genMs,
      ts: record.ts || "",
      ordinal: record.ordinal != null ? record.ordinal : null,
...
function buildStatsUnified(parsed, { retainRequests = true } = {}) {
  ...
  const createTurn = (start, fallbackTs, turnId = "") => {
    ...
    turns.push({
      ...
      total: 0,
      cumulative: false,
      // 本轮加权平均速度的分子/分母（与 lastRequestTps 同口径：逐请求输出 ÷ 逐请求生成耗时）
      genMsTotal: 0,
      genOutputTotal: 0,
    });
  ...
  // ---- 轮次列表：单一来源链 -------------------------------------------------
  // 旧实现把「有 turn_id 就新建轮」与「没有 turn_id 就用用户消息序号当数组下标」两套规则
  // 写进同一个数组，还会用 while (turns.length <= index) 填空轮；结果是
  //   ① legacy 记录可能落进「恰好排在那个下标的别的轮」；
  //   ② turns 尾部常是空轮，currentTurn = turns.at(-1) 于是恒为 0；
  //   ③ 同一轮被拆成「modern 轮 + boundary 轮」两份，面板那 5 格只剩速度在动。
  // 现在改为：
  //   1) 首选 turn_context（宿主逐轮写出，带 turn_id；实测覆盖到日志开头，且含无记录空轮）；
  //   2) 记录里出现、turn_context 未覆盖的 turn_id（更旧的日志可能没有该事件）；
  //   3) 完全没有 turn_id 线索 → 退回 user_message 时间分界（保持旧日志行为）；
  //   4) 连用户消息都没有 → 单轮。
  const seedIndexByTurnId = new Map();
  const turnSeeds = [];
  const addTurnSeed = (ts, ordinal, turnId) => {
    if (!turnId) return;
    const seed = { ts: ts || "", ordinal: Number.isFinite(ordinal) ? ordinal : null, turnId };
    const existing = seedIndexByTurnId.get(turnId);
    if (existing == null) {
      seedIndexByTurnId.set(turnId, turnSeeds.length);
      turnSeeds.push(seed);
      return;
    }
    if (compareTsOrdinal(seed, turnSeeds[existing]) < 0) turnSeeds[existing] = seed;
  };
  for (const context of parsed.turnContexts || []) {
    if (context && context.turnId) addTurnSeed(context.ts, context.ordinal, context.turnId);
  }
  for (const record of records) addTurnSeed(record.ts, record.ordinal, record.turnId);

  if (turnSeeds.length) {
    turnSeeds.sort(compareTsOrdinal);
    for (const seed of turnSeeds) {
      turnIndexById.set(seed.turnId, createTurn(seed.ts, seed.ts, seed.turnId));
    }
  } else if (starts.length) {
    for (const start of starts) createTurn(start.ts, start.ts);
  } else {
    createTurn("", records.length ? records[0].ts : "");
  }

  // 无 turn_id 的记录（legacy）按时间窗落到「开始时间 ≤ 记录时间」的最后一轮。
  const turnStarts = turns.map((turn) => turn.start || "");
  const locateTurnIndexByTs = (ts) => {
    if (!turns.length) return -1;
    if (!ts) return turns.length - 1;
    let low = 0;
    let high = turns.length - 1;
    let found = -1;
    while (low <= high) {
      const mid = (low + high) >> 1;
      const start = turnStarts[mid];
      if (!start || start <= ts) {
        found = mid;
        low = mid + 1;
      } else {
  ...
  const resolveTurnIndex = (record) => {
    if (record.turnId) {
      const mapped = turnIndexById.get(record.turnId);
      if (mapped != null) return mapped;
    }
    return locateTurnIndexByTs(record.ts);
  };

  for (const record of records) {
    const turnIndex = resolveTurnIndex(record);
    const turn = turns[turnIndex];
    const turnGenMs = Number.isFinite(record.genMs) ? record.genMs : 0;
    if (turnGenMs > 0) {
      turn.genMsTotal += turnGenMs;
      if (Number.isFinite(record.output) && record.output > 0) turn.genOutputTotal += record.output;
    }
  ...
  // 本轮加权平均速度：与 lastRequestTps 同口径，但把范围收在「当前这一轮」内。
  // 必须在这里算：payloadFor 拿到的是对外映射后的 turns（不含内部累加器）。
  const lastTurnForTps = turns.length ? turns[turns.length - 1] : null;
  const turnTps = lastTurnForTps && lastTurnForTps.genMsTotal > 0 && lastTurnForTps.genOutputTotal > 0
    ? lastTurnForTps.genOutputTotal / (lastTurnForTps.genMsTotal / 1000)
    : null;
...
function payloadFor(stats, health = makeHealth({ status: "healthy", dataFresh: true }), requestedWindow = null, sessionChanged = false) {
  // 窗口 = 本次推送包含多少条轮次摘要 / 请求明细。缺省按「弹层打开」的档位，保证非 CDP 路径（CLI）行为不变。
  const turnWindow = requestedWindow && Number.isFinite(requestedWindow.turns)
    ? Math.max(0, Math.min(MAX_TURN_SUMMARIES, Math.floor(requestedWindow.turns)))
    : TURN_WINDOW_OPEN;
  const requestWindow = requestedWindow && Number.isFinite(requestedWindow.requests)
    ? Math.max(0, Math.min(MAX_REQUEST_DETAILS, Math.floor(requestedWindow.requests)))
    : REQUEST_WINDOW_OPEN;
  ...
  const currentRequestTotal = currentTurn
    ...
    : 0;
  const requestSliceStart = Math.max(0, currentRequests.length - requestWindow);
  const requestIndexStart = Math.max(0, currentRequestTotal - currentRequests.length) + requestSliceStart;
  const turnStart = Math.max(0, stats.turns.length - turnWindow);
  ...
  const payload = {
    ...
    // 本轮加权平均速度（推导见 buildStats 内的 turnTps 注释）
    turnTps: Number.isFinite(stats.turnTps) ? stats.turnTps : null,
    ...
    // 下发窗口上限，让面板判断「已到上限」时不必再抄一份常量（避免隐藏耦合）
    turnsLimit: MAX_TURN_SUMMARIES,
    requestsLimit: MAX_REQUEST_DETAILS,
    // 一次性信号：本份负载对应的会话与上一份不同（供面板重置翻页位置）
  ...
  return new Promise((resolve) => {
    ...
    ws.onopen = () => {
      ws.send(JSON.stringify({
        ...
        params: {
          expression: "JSON.stringify({focused:document.hasFocus(),visible:document.visibilityState === 'visible',hasPanel:!!window.__tuiForCodexNativeUiInstalled})",
          returnByValue: true,
        },
      }));
    };
...
function assertSafeCdpExpression(expression) {
  if (typeof expression !== "string" || (!expression.startsWith("(function ()") && !expression.startsWith("JSON.stringify(") && !expression.startsWith("!!document.querySelector(") && !expression.startsWith("window.__tuiForCodex ="))) {
     ...
     port,
      `(function () {
        ...
        function rememberActiveThread(id) {
          window.__tuiForCodexActiveId = { id: id, at: Date.now() };
          return id;
        }
        ...
        if (!window.__tuiForCodexThreadObserverInstalled && document.documentElement && typeof MutationObserver === "function") {
          var observer = new MutationObserver(function (records) {
            for (var i = 0; i < records.length; i++) {
              ...
              if (relevant) {
                window.__tuiForCodexDomVersion = (window.__tuiForCodexDomVersion || 0) + 1;
                window.__tuiForCodexActiveIdCache = null;
                return;
              }
            }
          });
          ...
          window.__tuiForCodexThreadObserverInstalled = true;
          window.__tuiForCodexDomVersion = window.__tuiForCodexDomVersion || 0;
        }
        ...
        try {
          ...
          var now = Date.now();
          var cache = window.__tuiForCodexActiveIdCache;
          ...
          for (var a = 0; a < anchors.length; a++) {
            ...
            if (direct) { window.__tuiForCodexActiveIdCache = { id: direct, at: now }; return rememberActiveThread(direct); }
            for (var reactKey in anchor) {
              ...
              try {
                ...
                if (scanned) { window.__tuiForCodexActiveIdCache = { id: scanned, at: now }; return rememberActiveThread(scanned); }
              } catch (e) {}
            }
          }
          window.__tuiForCodexActiveIdCache = { id: null, at: now };
        } catch (e) {}
        // 侧边栏收起时 active/current 节点会消失；主会话区仍在时沿用最后确认的 ID。
        try {
          var last = window.__tuiForCodexActiveId;
...
async function cdpPush(port, payload, serialized = JSON.stringify(payload), targetSelector = DEFAULT_TARGET_SELECTOR) {
  const expr = `(function () {
    var protocol = window.__tuiForCodexProtocol;
    if (!window.__tuiForCodexNativeUiInstalled) return "page-not-ready";
...
function mergeSegments(parsedParts) {
  ...
  return {
    ...
    turnContexts: valid.flatMap((part) => part.turnContexts || []),
    // 运行状态是标量：取「最后一段里最后一次明确写过的值」
    taskRunning: valid.reduce((acc, part) => (typeof part.taskRunning === "boolean" ? part.taskRunning : acc), false),
    modelContextWindow,
...
// 读取页面声明的推送窗口（<html data-tui-window> = "turns:requests"）。
// 页面在「弹层开关」与「加载更多」时写入；监控每秒巡检时读一次，变化即立刻推送。
async function readWindowWant(port, targetSelector) {
  try {
    // 注意：cdpEval 有表达式白名单（必须以 JSON.stringify( / (function () 等开头），
    // 因此这里用 JSON.stringify 包一层再在本地解析；否则会被拦下、被 catch 吞成 null。
    const raw = await cdpEval(port, 'JSON.stringify(document.documentElement.getAttribute("data-tui-window") || "")', targetSelector);
    const value = JSON.parse(raw == null || raw === "" ? '""' : raw);
    const parts = String(value || "").split(":");
    const turns = Number.parseInt(parts[0], 10);
    const requests = Number.parseInt(parts[1], 10);
    if (!Number.isFinite(turns) || !Number.isFinite(requests)) return null;
...
function parseArgs(argv) {
  const envPort = Number(process.env.TUI_CDP_PORT || process.env.CODEX_CDP_PORT);
  const defaultPort = Number.isInteger(envPort) && envPort >= 1 && envPort <= 65535 ? envPort : 9229;
  const envTarget = String(process.env.TUI_CDP_TARGET || "").trim();
...
function createWatchSession(args) {
  return {
    ...
    lastKey: "",
    lastPushAt: 0,
    // 上一次生效的推送窗口（"turns:requests"），用于「窗口变化立刻推送」
    lastWindowKey: "",
    // 上次推送时使用的会话 ID（仅存于监控进程内存，不进负载）
    pushedThreadId: "",
    lastRealThreadId: "",
    lastStats: null,
    lastPayload: null,
...
async function publishWatchRound(args, session, context) {
  ...
  // 会话切换检测：解析出的会话与「上次推送时用的会话」不同 → 这一份负载带上 sessionChanged 一次。
  // 只下发布尔信号，不下发任何会话标识（线程 ID / 路径都不出监控进程）。
  const resolvedThreadId = stats.threadId || "";
  const sessionChanged = !!resolvedThreadId && !!session.pushedThreadId && session.pushedThreadId !== resolvedThreadId;
  if (resolvedThreadId) session.pushedThreadId = resolvedThreadId;
  ...
  // 生成中（task_started 尚未 task_complete）心跳 1 秒，其余 5 秒；窗口变化仍会立即推送。
  const activeInterval = context.parsed && context.parsed.taskRunning ? ACTIVE_HEARTBEAT_INTERVAL_MS : HEARTBEAT_INTERVAL_MS;
  // 展开态（窗口大于默认档）不参与心跳：用户正在看一份静态长列表，
  // 每秒重发它没有任何信息增量，却会把「最坏情况负载」放大到数百 KB/s。
  // 展开态只在数据/健康/窗口变化时推送（下面的 changed/healthChanged/windowChanged 仍然生效）。
  const expandedWindow = !!context.wantWindow && (context.wantWindow.turns > TURN_WINDOW_CLOSED || context.wantWindow.requests > REQUEST_WINDOW_CLOSED);
  const heartbeat = args.cdp && !expandedWindow && Date.now() - session.lastPushAt > activeInterval;
  const healthChanged = healthKeyOf(session.health) !== session.lastHealthKey;
  const windowKey = context.wantWindow ? context.wantWindow.turns + ":" + context.wantWindow.requests : "";
  const windowChanged = windowKey !== session.lastWindowKey;

  if (!(changed || heartbeat || connectionChanged || healthChanged || windowChanged || sessionChanged)) return;

  if (changed) session.lastKey = key;
  if (windowChanged) session.lastWindowKey = windowKey;

  if (args.cdp) {
    // 固定心跳也要重建负载以刷新 updatedAt；页面只在数据键变化时重绘，不会抖动
    if (changed || healthChanged || connectionChanged || heartbeat || windowChanged || !session.lastPayload) {
      session.lastPayload = payloadFor(stats, session.health, context.wantWindow, sessionChanged);
...
async function runWatchRound(args, session) {
  ...
  const wantWindow = args.cdp ? await readWindowWant(args.port, args.target) : null;
  const { thread } = await resolveWatchThread(args, session, rawThread);
  const { file, parsed, stats } = readWatchStats(session, thread);
  await publishWatchRound(args, session, { thread, file, parsed, stats, rawThread, connection, connectionState, wantWindow });
}
...
if (!process.env.TUI_TOKENS_AS_MODULE) {
  main().catch((e) => {
