...
| Node.js 22+ | `node --version`，或 `%USERPROFILE%\.cache\codex-runtimes` 下存在 `node.exe` | 二选一即可；都没有则装 Node |
| PowerShell 5.1+ | 系统自带 | — |

说明：Node 可省略——Codex 桌面版自带运行时（实测 v24）会被自动复用；
`find-codex.ps1` 的查找顺序是 PATH → Codex 自带运行时 → 常见安装位置。

## 1. 需要带走的东西

只需要**一个部署包目录**（包含 `install.bat`、`install.ps1`、`.agents/`、`plugins/`）。

**不需要**带走：状态目录（`%LOCALAPPDATA%\tokens-ui-for-codex\`）、日志、会话映射。
这些会在新机器上重新生成；旧机器的历史数据不迁移（也不建议迁移，避免会话映射错配）。

如果需要保留旧机器的日志做对照，可单独复制 `logs\` 目录，但不要复制
`client-thread-map.json`（它记录旧机器的线程 ID 映射）。
...
检查 Codex++ 与其用户脚本总开关、写入用户脚本（备份+哈希校验）、注册插件、
...
# ③ 用户脚本哈希（应与包内文件一致）
Get-FileHash "$env:APPDATA\Codex++\user_scripts\codex-token-spend-panel.js"
Get-FileHash "<包根>\plugins\tokens-ui-for-codex\codex-token-spend-panel.js"
...
| 现象 | 原因 | 处置 |
| --- | --- | --- |
| 安装器报"未找到 codex 命令" | 桌面版是 MSIX，bin 不在 PATH | 安装器已自动搜索；若仍失败，检查 `%LOCALAPPDATA%\OpenAI\Codex\bin` 是否存在 |
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File "<包根>\plugins\tokens-ui-for-codex\uninstall-autostart.ps1" -Full
```

`-Full` 会清除计划任务、进程、状态目录、用户脚本、插件与插件缓存；最后手动删除包目录。

## 6. 权限说明（迁移相关）

- 全程不需要管理员权限，不弹 UAC；
- 计划任务注册在**当前用户**下（登录触发 + 每 5 分钟巡检 + 安装后立即执行）；
- 不提供"未登录也运行"：那需要管理员或存储凭据，本插件明确不使用；
- 进程检测依赖 `Get-CimInstance` 读取命令行，对其他用户/提权进程可能读不到（已做容错，只影响检测精度，不影响运行）。
