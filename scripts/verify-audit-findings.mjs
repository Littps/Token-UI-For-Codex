...
{
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "tui-audit-f1-"));
...
// ---- F3：协议死代码检查 ----
{
  ...
  const panelSource = fs.readFileSync(new URL("../tokens-ui-for-codex-panel.js", import.meta.url), "utf8");
...
{
  const tmp = os.tmpdir();
  const leftovers = fs.readdirSync(tmp).filter((name) => name.startsWith("tokens-ui-test-"));
