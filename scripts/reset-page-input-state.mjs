...
const snapshot = () =>
  page.evaluate(() => ({
    probeElementExists: !!document.getElementById("__ccm_input_probe"),
    probeGlobalExists: typeof window.__ccmInputProbe === "object" && window.__ccmInputProbe !== null,
...
await page.evaluate(() => {
  document.getElementById("__ccm_input_probe")?.remove();
  try {
    delete window.__ccmInputProbe;
  } catch {}
});
