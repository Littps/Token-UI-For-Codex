...
$panelSource = Join-Path $pluginRoot "tokens-ui-for-codex-panel.js"
$statsScript = Join-Path $pluginRoot "token-stats.mjs"
$userScripts = Join-Path $env:APPDATA "Codex++\user_scripts"
$userScriptTarget = Join-Path $userScripts "tokens-ui-for-codex-panel.js"
...
if (Test-Path -LiteralPath $userScripts) {
  ...
  # 停用同目录下的其它历史面板脚本：按脚本内容识别，不看文件名。
  # 必要性：Codex++ 会加载 user_scripts 里的所有 .js，旧文件若留着重启后会与新面板
  # 同时注入，页面上出现两条统计条。改名备份（.superseded-*.bak）不会被加载。
  $supersededStamp = Get-Date -Format "yyyyMMdd-HHmmss"
  foreach ($file in @(Get-ChildItem -LiteralPath $userScripts -File -Filter *.js -ErrorAction SilentlyContinue)) {
    if ($file.FullName -eq $userScriptTarget) { continue }
    $isOurPanel = $false
    try { $isOurPanel = (Get-Content -LiteralPath $file.FullName -Raw -ErrorAction SilentlyContinue) -match "NativeUiInstalled" } catch { $isOurPanel = $false }
    if (-not $isOurPanel) { continue }
    $retired = $file.FullName + ".superseded-" + $supersededStamp + ".bak"
    Move-Item -LiteralPath $file.FullName -Destination $retired -Force -ErrorAction SilentlyContinue
    if (Test-Path -LiteralPath $retired) { Write-Note ("已停用历史面板脚本（改名备份）：" + $file.Name) }
    else { Write-Bad ("历史面板脚本停用失败，可能与应用内重复统计条共存：" + $file.Name) }
  }
