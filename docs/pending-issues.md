...
## P3. 锚点父元素恰为挂载容器时，可能每帧重挂载（已定位，未修复；当前 Codex 结构下不触发）
...
**修复方向（待执行）**：把「是否需要重新插入」的判据归一化 ——
`before` 有效时用 `nextSibling === before`；`before` 降级为 `null` 时用
`state.root === point.container.lastElementChild` 替代，使循环可收敛。
