...
const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "ccm-token-spend-test-"));
...
test("checked JSON schema documents the same protocol contract", () => {
  ...
  assert.equal(schema.properties.requestDetails.maxItems, 100);
  assert.equal(schema.properties.turns.maxItems, 200);
});
