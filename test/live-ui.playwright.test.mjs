...
//    只有显式设置 CCM_ALLOW_REAL_INPUT=1 时才执行。
...
const CDP_ENDPOINT = process.env.CCM_CDP_URL || "http://127.0.0.1:9229";
const PANEL_SELECTOR = "#ccm-token-spend-native";
const ALLOW_REAL_INPUT = process.env.CCM_ALLOW_REAL_INPUT === "1";
const realInputSkip = ALLOW_REAL_INPUT
  ? false
  : "默认跳过真实输入注入：Escape 会被 Codex 当作“停止回答”，需要显式设置 CCM_ALLOW_REAL_INPUT=1";
...
async function openDetails() {
  ...
  return state.page.evaluate(() => !!document.getElementById("ccm-token-spend-detail-dialog"));
}
...
async function readDialog() {
  return state.page.evaluate(() => {
    ...
    const firstSection = dialog.querySelector(".ccm-ts-dialog-section");
    return {
      open: dialog.getAttribute("data-open") === "true",
      role: dialog.getAttribute("role"),
      titles: [...dialog.querySelectorAll(".ccm-ts-dialog-section-title")].map((node) => node.textContent),
      firstSectionLabels: firstSection
        ? [...firstSection.querySelectorAll(".ccm-ts-dialog-card-label")].map((node) => node.textContent)
        : [],
    };
  });
}
...
async function closeIfOpen() {
  const closed = await state.page.evaluate(() => {
    if (!document.getElementById("ccm-token-spend-detail-dialog")) return true;
    document.body.dispatchEvent(
      new PointerEvent("pointerdown", { bubbles: true, cancelable: true, pointerType: "mouse" }),
    );
    if (document.getElementById("ccm-token-spend-detail-dialog")) {
      document.body.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, cancelable: true }));
    }
    if (document.getElementById("ccm-token-spend-detail-dialog")) {
      const close = document.querySelector("#ccm-token-spend-detail-dialog .ccm-ts-dialog-close");
...
test("实机：统计条唯一且位于上下文用量圆圈左侧", async (t) => {
  const info = await state.page.evaluate(() => {
    ...
    const legacyCount = ["ccm-token-spend-panel", "ccm-token-spend-mini", "ccm-token-spend-style"].filter((id) =>
    ...
    return {
      ...
      labels: root ? [...root.querySelectorAll(".ccm-ts-native-label")].map((node) => node.textContent) : [],
    };
  });
  t.diagnostic(JSON.stringify(info));
  assert.equal(info.panelCount, 1, "统计条数量应为 1");
  assert.equal(info.legacyCount, 0, "不应存在旧面板残留");
...
test("实机：负载协议与隐私白名单", async (t) => {
  const payload = await state.page.evaluate(() => window.__ccmTokenSpend || null);
...
test("实机：每 5 秒固定刷新", async (t) => {
  const first = await state.page.evaluate(() => window.__ccmTokenSpend && window.__ccmTokenSpend.updatedAt);
  await new Promise((resolve) => setTimeout(resolve, 6500));
  const second = await state.page.evaluate(() => window.__ccmTokenSpend && window.__ccmTokenSpend.updatedAt);
  const deltaMs = Date.parse(second) - Date.parse(first);
  t.diagnostic(JSON.stringify({ first, second, deltaMs }));
  assert.ok(deltaMs >= 3000 && deltaMs <= 9000, "刷新间隔不在 3~9 秒范围: " + deltaMs);
});
...
test("实机：详情分区顺序与会话累计请求数量", async (t) => {
  ...
  const healthLabels = await state.page.evaluate(() => {
    const dialog = document.getElementById("ccm-token-spend-detail-dialog");
    if (!dialog) return [];
    const section = [...dialog.querySelectorAll(".ccm-ts-dialog-section")].find(
      (item) => item.querySelector(".ccm-ts-dialog-section-title")?.textContent === "运行状态",
    );
    return section ? [...section.querySelectorAll(".ccm-ts-dialog-card-label")].map((node) => node.textContent) : [];
  });
...
test("实机：上下文使用显示实际上限而非“未知”", async (t) => {
  ...
  const context = await state.page.evaluate(() => {
    const dialog = document.getElementById("ccm-token-spend-detail-dialog");
    if (!dialog) return null;
    const sections = [...dialog.querySelectorAll(".ccm-ts-dialog-section")];
    const target = sections.find(
      (section) => section.querySelector(".ccm-ts-dialog-section-title")?.textContent === "上下文使用",
    );
    if (!target) return { found: false };
    const muted = target.querySelector(".ccm-ts-dialog-muted");
    const used = target.querySelector(".ccm-ts-dialog-context-value");
    return {
      found: true,
      limitText: muted ? muted.textContent : null,
      usedText: used ? used.textContent : null,
      fillWidth: target.querySelector(".ccm-ts-dialog-context-fill")?.style?.width || null,
      payloadWindow: window.__ccmTokenSpend ? window.__ccmTokenSpend.modelContextWindow : null,
    };
  });
...
test("实机：点击弹层内部不会关闭", async (t) => {
  ...
  const stillOpen = await state.page.evaluate(() => {
    const dialog = document.getElementById("ccm-token-spend-detail-dialog");
    const body = dialog ? dialog.querySelector(".ccm-ts-dialog-body") : null;
    if (body) {
...
test("实机：无效负载会被页面拒绝", async (t) => {
  const result = await state.page.evaluate(async () => {
    const backup = window.__ccmTokenSpend;
    window.__ccmTokenSpend = { protocolName: "tokens-ui-for-codex", schemaVersion: 2, bogus: true };
    window.dispatchEvent(new Event("ccm-token-spend"));
    await new Promise((resolve) => setTimeout(resolve, 100));
    const root = document.getElementById("ccm-token-spend-native");
    const panelError = root ? root.classList.contains("ccm-ts-native-error") : null;
    const ariaLabel = root ? root.getAttribute("aria-label") : null;
    window.__ccmTokenSpend = backup;
    window.dispatchEvent(new Event("ccm-token-spend"));
